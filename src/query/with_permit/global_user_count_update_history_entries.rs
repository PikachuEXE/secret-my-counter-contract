...
pub fn query_global_user_count_update_history_entries(deps: Deps, page_one_based: u32, page_size: u32, suffix_4_test: Option<&[u8]>) -> StdResult<QueryAnswer> {
    let entries = UserCountUpdateHistoryManager::get_global_entries(deps.storage, page_one_based - 1, page_size, suffix_4_test);
...
mod tests {
    ...
    fn query_global_user_count_update_history_entries_works() -> StdResult<()> {
        ...
        // save
        assert!(UserCountUpdateHistoryManager::add_entry(deps.as_mut().storage, env.clone(), UserCountUpdateHistoryEntry {
        ...
        // actual query
        assert_eq!(query_global_user_count_update_history_entries(deps.as_ref(), 1, 2, Some(suffix_4_test))?, QueryAnswer::UserCountUpdateHistoryEntries {
            entries: vec![
