...
pub fn query_user_count_update_history_entries(deps: Deps, viewer: String, page_one_based: u32, page_size: u32) -> StdResult<QueryAnswer> {
    let entries = UserCountUpdateHistoryManager::get_user_entries(deps.storage, deps.api.addr_validate(viewer.as_str())?, page_one_based - 1, page_size);
...
mod tests {
    ...
    fn query_user_count_update_history_entries_works() -> StdResult<()> {
        ...
        // actual query
        assert_eq!(query_user_count_update_history_entries(deps.as_ref(), user_addr.to_string(), 1, 1)?, QueryAnswer::UserCountUpdateHistoryEntries {
            entries: vec![
        ...
        // Fail query
        assert_eq!(query_user_count_update_history_entries(deps.as_ref(), "not_user_addr".to_string(), 1, 1)?, QueryAnswer::UserCountUpdateHistoryEntries {
            entries: vec![],
        });
